...
export default function CreateScheduleModal({ vpsId, onClose, onSuccess }: CreateScheduleModalProps) {
  ...
  const { data: containersData } = useQuery({
    queryKey: ['docker-containers', vpsId],
    queryFn: () => dockerApi.getContainers(vpsId, true),
  });

  const createMutation = useMutation({
    mutationFn: (data: any) => dockerScheduleApi.create(vpsId, data),
    onSuccess: () => {
      onSuccess();
    },
    onError: (error: any) => {
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
        <div className="flex items-center justify-between pb-3">
          <h3 className="text-lg font-medium text-gray-900">Create Docker Schedule</h3>
          <button
            onClick={onClose}
